...
public class VentaController {
    ...
    @GetMapping("/{number}")
    public ResponseEntity<?> devolverVenta(@PathVariable String number){
        return new ResponseEntity<>(ventaService.devolverVenta(number), HttpStatus.OK);
    }

    @PutMapping("/{number}")
    public ResponseEntity<?> actualizarVenta(@PathVariable String number, @RequestBody VentaRequestDto venta){
        return new ResponseEntity<>(ventaService.actualizarVenta(number, venta), HttpStatus.OK);
    }

    @DeleteMapping("/{number}")
    public ResponseEntity<?> eliminarVenta(@PathVariable String number){
        return new ResponseEntity<>(ventaService.eliminarVenta(number), HttpStatus.OK);
    }

    @GetMapping("/clothes/{number}")
    public ResponseEntity<?> listarPrendasVenta(@PathVariable String number){
