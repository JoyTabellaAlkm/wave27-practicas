...
@Service
public class CodificarCodigoMorse implements ICodigoMorseService {
    ...
    public String decodificador(String codigo) {
        ...
        String[] dividiendoCodigoMorse = codigo.split(" ");

        List<String> mensaje =  new ArrayList<>();

        for (int i = 0; i < dividiendoCodigoMorse.length; i++) {
            for (Map.Entry<String, String> entry : alphaMorse.entrySet()) {
                if (entry.getKey().equals(dividiendoCodigoMorse[i])) {
                    mensaje.add(entry.getValue());
                }
            }
        }
        //String[] dividiendoCodigoFrase = codigo.split("   ");
        return mensaje.toString().replaceAll("\\[|\\]", "").replaceAll(", ", " ");
    }
}
