...
SELECT ser.title, COUNT(sea.id) AS total_temporadas FROM series ser
INNER JOIN seasons sea ON ser.id = sea.serie_id
GROUP BY ser.title;

# Mostrar el nombre de todos los géneros y la cantidad total de películas por cada uno, siempre que sea mayor o igual a 3.
SELECT g.name, COUNT(m.genre_id) AS total_peliculas FROM genres g
INNER JOIN movies m ON g.id = m.genre_id
GROUP BY g.name
HAVING total_peliculas >= 3;

# Mostrar sólo el nombre y apellido de los actores que trabajan en todas las películas de la guerra de las galaxias y que estos no se repitan.
SELECT DISTINCT CONCAT(a.first_name, ' ', a.last_name) AS nombre_apellido FROM actors a
